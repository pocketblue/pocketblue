...
Things we are working around rather than fixing, worth revisiting when upstream
moves:

- **blob-wrangler [#4](https://github.com/samcday/blob-wrangler/issues/4)** (open),
  first item: folder extraction is not idempotent. An existing folder is not
  overwritten, is then missing from the new `status.json`, and is deleted as
  stale on the run after that - so the sensor tree came and went on alternate
  boots, and a boot without it crashes the ADSP.

  Fixed in our fork ([pocketblue/blob-wrangler](https://github.com/pocketblue/blob-wrangler),
  branch `pocketblue-fixes`, built in `@mobility/common`), which also allows a
  config with no `firmware` entries instead of panicking. The fork is based on
  upstream PR #13, so it carries the slot-safe extraction too. Switch back to
  upstream once both land.
- **blob-wrangler #4**, third item: MDT squashing ignores `read_exact` errors
  after pre-sizing the buffer, so a failed read can silently produce
  zero-filled `.mbn` output. Every DSP blob on this device goes through that
  path, and the failure would look like a firmware crash rather than a bad
  file. Worth suspecting if the DSPs misbehave after a clean extraction.
- **blob-wrangler [#13](https://github.com/samcday/blob-wrangler/pull/13)**
  (open PR): no active-slot detection. The log says
  `Active slot is unknown; preserving legacy _a-then-_b selection`, so this
  device always reads slot A. We pin the nightly build because it already
  carries this work; firmware from the inactive slot would otherwise be a real
  possibility.
- **blob-wrangler [#11](https://github.com/samcday/blob-wrangler/issues/11)**
  (open): `make-dynpart-mappings` is not a dependency of the package, and
  blob-wrangler hard-fails if its unit is absent, without checking whether the
  mappings already exist. We install it explicitly from `pocketblue/common`.
- **The ADSP needs a populated sensor registry.** `sensors/registry` comes from
  the `persist` partition and holds ~114 per-sensor files. Without it the ADSP
  crashes in `SNS_REG_TASK` (`sns_registry_sensor.c:154`) over and over, and the
  kernel sometimes oopses recovering from that, which shows up as an
  intermittent boot loop. blob-wrangler used to delete this folder on every
  re-run, which is why it kept coming back; our fork fixes that. The reference
  layout is FairBlobs' `hexagonfs/sensors`: `config/`, `registry/`,
  `sns_reg.conf`.

- **Kernel, ADSP recovery**: an ADSP crash can take the whole kernel down. The
  chain is `fatal error received: EX:kernel:0x1:sns_sig_remote_` ->
  `crash detected in adsp` -> `recovering adsp` ->
  `Unable to handle kernel NULL pointer dereference at 0000000000000348` ->
  `Internal error: Oops`, and the machine reboots. Recovery sometimes succeeds,
  which is why this shows up as an intermittent boot loop rather than a
  reliable failure.

  The DSP-side trigger is a missing sensor configuration - hexagonrpcd logging
  `Could not open /vendor/etc/sensors/sns_reg_config` is the tell - so keeping
  the extraction correct avoids it. But a crashing DSP should never be able to
  oops the kernel, so the recovery path is a genuine bug worth reporting
  upstream. Not reported yet.
- **soundwire**: `din-ports/dout-ports mismatch with controller` on every boot,
  including healthy ones. Appeared with 7.1.2 and looks unrelated to firmware.


### Kernel

Pocketblue uses a Fedora ark-based kernel (via arkify) with additional patches for Qualcomm SC7280 (QCM6490/Kodiak) support. It is built in the [@mobility/sc7280 COPR repository](https://copr.fedorainfracloud.org/coprs/g/mobility/sc7280/) from [LorbusChris/linux-sc7280](https://github.com/LorbusChris/linux-sc7280), branch `sc7280-7.1.y`, via `make_srpm`.
